"""
Main controller: poll -> download -> process -> upload -> cleanup
"""
import time
import logging
import os
...
from driver import is_video_cc, download_video
...
def cleanup_temp_files(video_id: str, tmp_dir: str):
    """
    Clean up only temp files related to a specific video (wav, srt, tts, etc.).
    """
    try:
        for f in os.listdir(tmp_dir):
            if video_id in f:  # match files that belong to this video
                try:
        ...
        for vid in new_vids:
            try:
                ...
                if not ok:
                    ...
                    mark_processed(vid)
                    continue
                local = download_video(vid, TMP)
                if not local:
                    logging.error('Download failed for %s', vid)
                    mark_processed(vid)
                    continue
                # Process
                final = process_video_file(local)
                if not final:
                ...
                upload_video_to_youtube(yt_service, final, title, desc)

                # cleanup local artifacts
                try:
                    os.remove(local)  # remove original downloaded video
                except:
                    pass

                # cleanup temp files linked to this video
                cleanup_temp_files(Path(local).stem, TMP)
